...
size_mod_out <- as.data.frame(size_ci)[5:9,] %>% 
  ...
  mutate(x_lab = case_when(sample == "subsampleb10" ~ "10",
                           sample == "subsampleb20" ~ "20",
                           sample == "subsampleb30" ~ "30",
                           sample == "subsampleb40" ~ "40",
                           sample == "subsampleb50" ~ "50"))

ggplot(size_mod_out)+
  geom_point(aes(x = x_lab, y = est), size = 3) +
  geom_errorbar(aes(x = x_lab, ymin = `2.5 %`, ymax = `97.5 %`), width = 0.3, size = 1.5)+
  ylab("log(niche size)")+
  xlab("Animal locations per week")+
  theme_classic()
