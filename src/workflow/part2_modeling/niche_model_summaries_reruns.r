#!/usr/bin/env Rscript 
# Plot model summary sheet (PDF) for niche models that needed to be rerun
# based on the output PDFs of niche_model_summaries.r. Select either the 
# additive or interactive model based on significance. Note that this script
# and area_model_summaries_reruns.r were used to check the model reuns, but
  ...
  if(int_modlist_full[i] != "NULL"){
    ...
    if(fe$pd[fe$Parameter=="b_sg_norm:ghm_scale"] < 0.95 & fe$pd[fe$Parameter=="b_sg_norm:ghm_scale"] > 0.05){ # if the interacton effect is non-sig...
      ...
      if(add_modlist_full[i] != "NULL"){
        ...
        # Write out plot
        ggsave(model_out, filename = glue("{.outPF}/model_diagnostics_reruns/niche/{sp}.pdf"), 
